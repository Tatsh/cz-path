...
When a managed file is modified directly (e.g. `uv add`, manual `pyproject.toml` edits), you update
`.wiswa.jsonnet` so that running Wiswa again would produce the same result.
...
Structured files (YAML, TOML, JSON) map directly to top-level keys in `.wiswa.jsonnet` with the same
key structure. No key renaming or transformation is needed.
...
2. **Read the changed managed files** (e.g. `pyproject.toml`, `package.json`) to identify what
   changed.
