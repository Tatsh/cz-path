...
Fixes mypy type errors and eliminates `Any` usage in the cz-path project.
...
1. Fix all mypy errors reported by `uv run mypy cz_path`
...
Place new TypedDict classes in `cz_path/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`cz_path/typing.py` when shared.
...
1. Run `uv run mypy cz_path` and capture all errors.
...
4. Run `uv run mypy cz_path` again to verify fixes.
