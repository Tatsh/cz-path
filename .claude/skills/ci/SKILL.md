...
If any changed files are under `cz_path/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `cz_path/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `cz_path/commands/main.py`,
...
For Python files, strip the `cz_path/` prefix and replace `/` with `.` (like module imports).

- Python file `cz_path/media.py` → `media:`.
- Multiple files under `cz_path/commands/` → `commands:`.
- Single command file `cz_path/commands/admin.py` → `commands.admin:`.
